...
코드를 실행한 결과, int형의 범위를 넘는 값이 입력된 경우 overflow가 발생해 음수가 되면서 Runtime Error가 발생했습니다.

이를 해결하기 위해 두 번째 방법을 사용했습니다.
...
두 연결 리스트에 대해 반복문을 돕니다.
두 연결 리스트의 순회가 모두 끝나면 반복문에서 탈출하고, 한쪽이 먼저 끝나면 그 숫자를 0으로 가정합니다.
...
두 수를 더해 합을 계산합니다.
만약 두 수의 합이 **10 이상**이라면 *올림*이 발생한 것이므로 10으로 나누어 올림(carry)을 계산하고,
10으로 나눈 나머지를 계산해 해당 자릿수를 구합니다.

발생한 올림은 다음 자릿수 계산에 더해줍니다.
...
반복문에서 탈출한 후, 올림이 남아 있다면 해당 숫자도 자릿수에 추가합니다.
