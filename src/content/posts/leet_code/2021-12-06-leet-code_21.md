...
description: 정렬되어 있는 두 연결 리스트를 하나로 합쳐야 합니다.
---
...
새로운 연결 리스트를 구성할 때는 항상 마지막에 노드를 삽입해야 하기 때문에 별도의 tail 포인터를 사용합니다.
...
이제 두 리스트를 동시에 순회합니다. 두 리스트의 노드 크기를 비교해야 하기 때문에 동시에 순회해야 합니다.

만약 한쪽 리스트의 노드가 남는다면, 어차피 정렬되어 있는 리스트이기 때문에 그대로 붙이기만 하면 됩니다.
...
첫 번째로는 새로운 연결 리스트에 붙일 노드를 선택합니다. 새로운 연결 리스트도 오름차순으로 정렬되어 반환되어야 하기 때문에 두 노드 중 크기가 작은 노드를 선택합니다.
...
그리고 노드를 새로운 연결 리스트에 삽입합니다. 만약 새 연결 리스트의 head가 nullptr라면 첫 순회이므로 그냥 삽입하고, 아니라면 tail 노드의 next 포인터가 가리키게 합니다.
...
크게 어려운 문제는 아니었기에, 다행히 충분히 만족스러운 속도가 나왔습니다.
