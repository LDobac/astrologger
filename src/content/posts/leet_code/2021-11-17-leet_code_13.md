...
로마자 숫자를 정수로 바꾸는 방법은 매우 쉽게 유도할 수 있습니다.

첫 번째로, 각 로마자 심볼에는 정해진 값이 있습니다. 이 값을 1:1로 매칭하기 위해 std::map STL 자료형을 사용합니다.

두 번째로, 4, 9와 같은 수는 IV, IX처럼 5, 10을 나타내는 심볼 앞에 1을 나타내는 심볼이 붙어서 표현됩니다. 그리고 로마자 숫자를 살펴보면 4, 9와 같은 경우를 제외하고는, 작은 심볼이 큰 심볼보다 앞에 오는 경우가 없습니다.

예를 들어 `LVIII`는 58을 나타내며 L은 50, V는 5, I는 1입니다. `MCMXCIV`는 1994를 나타내며 M은 1000, CM은 900, XC는 90, IV는 4입니다.
...
추측하건대, std::map STL 자료형을 사용하면서 메모리 생성 과정과 접근 과정에서 많은 시간을 소요하는 것 같습니다.
...
방법 1에서는 std::map을 이용해 로마자 숫자를 정수형으로 변환했지만, 이번에는 이 부분을 switch문을 이용한 하드 코딩으로 바꿔서 실행해보겠습니다.
